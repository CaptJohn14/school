...
static real_T emlrt_marshallIn(const emlrtStack *sp, const mxArray *w, const
  char_T *identifier);
  ...
  char_T *identifier)
{
  ...
  y = b_emlrt_marshallIn(sp, emlrtAlias(w), &thisId);
  emlrtDestroyArray(&w);
  return y;
}
...
void test_api(const mxArray * const prhs[1], int32_T nlhs, const mxArray *plhs[1])
{
  ...
  w = emlrt_marshallIn(&st, emlrtAliasP(prhs[0]), "w");
