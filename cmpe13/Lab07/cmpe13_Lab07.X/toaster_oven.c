        ...
        switch (kitchen.state) {
            ...
            case START:
                if (AdcChanged()) {//if potentiometer changed
                    if (kitchen.input == TIME) {
                        ...
                        //                        kitchen.initial_time = kitchen.initial_time >>1;
                        kitchen.initial_time = kitchen.initial_time << 1;
                        kitchen.timeleft = kitchen.initial_time;
                    } else {
                ...
                if (btnevent == BUTTON_EVENT_4DOWN) {//if button 4 pressed
                    ...
                    hz2 = FALSE;
                    //initialtime = kitchen.initial_time;   //can't figure out how to do the div 2 for timer..
                    //initialtime = kitchen.initial_time >> 1;
                    //freetime = kitchen.timeleft >> 1;
            ...
            case PENDING_SELECTOR_CHANGE:
                if (kitchen.btncounter < LONG_PRESS && btnevent == BUTTON_EVENT_3UP) {
                    kitchen.initial_time = 1; //reset time
                    kitchen.timeleft = 1; //reset time. don't think i need this. but why not
                    kitchen.input = TIME; //reset input selection
                    if (kitchen.cookmode == BAKE) {//rotate cooking mode
                    ...
                    btnevent = BUTTON_EVENT_NONE;
                    kitchen.state = START;
                } else if (kitchen.btncounter >= LONG_PRESS) {//if held button for 1 sec, change input.
                    if (kitchen.input == TIME) {
                        kitchen.input = TEMP;
                    } else {
                        kitchen.input = TIME;
...
void LedCook(void) {
    ...
    //        LEDS_SET(0xFF);
    //    }
    int temp = (int) ((kitchen.timeleft * 8 / kitchen.initial_time));
    if (temp == 8) {
        LEDS_SET(0xFF);
    } else if (temp == 7) {
        LEDS_SET(0xFE);
