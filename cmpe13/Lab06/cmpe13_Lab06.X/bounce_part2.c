...
int main(void) {
    ...
    /*Use the functions in Oled.h (OledDrawString() specifically) to display the current
    ...
    complete 0-100% range. Note: You will need the Oled files for this to work.*/

    //test to print to oled
    //OledInit();
    //    OledDrawString("testing testing");
    //    OledUpdate();
    ...
    while (TRUE) {
        if (eventData.event == TRUE) {
            temp = eventData.value * 100 / 1023;
            percent = (int) temp;
            sprintf(update, "Potentiometer value:\n%4d\n%3d%%", eventData.value, percent);
            OledDrawString(update);
            OledUpdate();
        }
        eventData.event = FALSE;
    }
...
void __ISR(_ADC_VECTOR, IPL2AUTO) AdcHandler(void) {
    ...
    /*Read the 8 buffered ADC values in the ADC1BUF0 through ADC1BUF7
variables and average all of them. If the resultant value is different than
the last averaged reading of the ADC, store it in the value member of
the AdcResult struct and set the event member to true.*/
    //int average;  
    double average = (double) ((ADC1BUF0 + ADC1BUF1 + ADC1BUF2 + ADC1BUF3 + ADC1BUF4 + ADC1BUF5 + ADC1BUF6 + ADC1BUF7) / 8);
    if (average != eventData.value) {
        eventData.event = TRUE;
        eventData.value = average;
    }
}