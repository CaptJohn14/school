...
ES_Event RunSubstateFINDIFZ(ES_Event ThisEvent) {
    ...
    switch (CurrentState) {
        ...
        case NOTAPEOBSTACLE: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                case ES_NO_EVENT:
                default: // all unhandled events pass the event back up to the next level
                    ES_Timer_InitTimer(FINDIFZ_TIMER, TIMER_TICKS);
                    nextState = ONEEIGHTYTURN;
                    Motors_SetRight(0);
                    Motors_SetLeft(0);
                    makeTransition = TRUE;
                    ThisEvent.EventParam = ES_NO_EVENT;
        ...
        case ONEEIGHTYTURN: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                case ES_TIMEOUT:
        ...
        case TRACKTAPE: // in the first state, replace this with correct names
            ThisEvent = RunSubstateTRACKTAPE(ThisEvent);
            switch (ThisEvent.EventType) {
                case BUMPER_PRESS:
                    bumper = ThisEvent.EventParam;
                    if (bumper == 1) {
                        rightturnflag = 1;
        ...
        case OBSTACLETURN: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                case ES_TIMEOUT:
                    if (ThisEvent.EventParam == FINDIFZ_TIMER) {
                        ES_Timer_InitTimer(FINDIFZ_TIMER2, TIMER_TICKS);
                        nextState = MOVEHALFFIELD;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    }
                    break;
        ...
        case MOVEHALFFIELD: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                case ES_TIMEOUT:
