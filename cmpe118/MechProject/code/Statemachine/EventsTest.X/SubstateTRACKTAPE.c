...
//                        Motors_SetBoth(15);
        ...
        case LEFT_TURN_FIRST: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                    ...
                    //                    break;
                case TAPE_OffToOn:
                    if ((ThisEvent.EventParam & FRB_TAPE_MASK)) {
                        nextState = TRACK_FRB;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    }
                    break;
                case ES_ENTRY:
                    Motors_SetRight(Hard_Turn); //soft left turn
                    Motors_SetLeft(0);
                    break;
                case ES_NO_EVENT:
                default: // all unhandled events pass the event back up to the next level
        ...
        case LTURN_FRB: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                    ...
                    //                    break;
                case TAPE_OffToOn:
                    if ((ThisEvent.EventParam & FR_TAPE_MASK)) {
                        nextState = TRACK_FRB;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    }
        ...
        case NINETYTURN_R: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                ...
                case ES_ENTRY:
                    ...
                    Motors_SetLeft(-Hard_Turn); // 90degree turn left
                    break;
                case ES_NO_EVENT:
                default: // all unhandled events pass the event back up to the next level
                    //                    Motors_SetRight(100);
        ...
        case RIGHT_TURN_FIRST: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                    ...
                    //                    break;
                case TAPE_OffToOn:
                    if ((ThisEvent.EventParam & FLB_TAPE_MASK)) {
                        nextState = TRACK_FLB;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    }
        ...
        case RTURN_FLB: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                    ...
                    //                    break;
                case TAPE_OffToOn:
                    if ((ThisEvent.EventParam & FL_TAPE_MASK)) {
                        nextState = TRACK_FLB;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    }
        ...
        case NINETYTURN_L: // in the first state, replace this with correct names
            switch (ThisEvent.EventType) {
                ...
                case ES_ENTRY:
                    Motors_SetRight(-Hard_Turn);
                    Motors_SetLeft(Hard_Turn); // 90degree turn right
                    break;
                case ES_NO_EVENT:
                default: // all unhandled events pass the event back up to the next level
                    //                    Motors_SetRight(-100);
